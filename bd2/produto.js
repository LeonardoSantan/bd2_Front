...
document.addEventListener("DOMContentLoaded", () => {
    const urlProdutoIndividual = `https://smsfranciscobeltrao.com.br/produtos/${localStorage.getItem('currentProduto')}/`;
    const userId = localStorage.getItem("id");

    axios.defaults.headers.common["user"] = userId;

    axios.get(urlProdutoIndividual)
        .then((response) => {
...
document.getElementById("btnComprar").addEventListener("click", () => {
    ...
    const quantidadeVenda = document.querySelector('#quantidadeVenda').value; // Captura a quantidade inserida pelo usuário
    const formaPagamento = document.querySelector('#formaPagamento').value;

    const dadosCompra = {
        codigo_produto: codigoProduto,
        quantidade_a_ser_vendido: quantidadeVenda,
        usuario: userId,
        forma_pagamento: formaPagamento
    };

    axios.post(urlCompra, dadosCompra)
        .then((response) => {
